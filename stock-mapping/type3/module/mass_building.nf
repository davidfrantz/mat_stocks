...
workflow mass_building {
    ...
    // tile, state, file_area, file_volume, type, material, mi_area, mi_volume
    sdr_hr = multijoin([sdr_hr_area, sdr_hr_volume], [0,1] )
    .combine( Channel.from("sdr_hr") )
    .combine( mi.map{ tab -> [tab.material, tab.sdr_hr_area, tab.sdr_hr_volume] } )
    ...
    // tile, state, file_area, file_volume, type, material, mi_area, mi_volume
    dcmix_hr = multijoin([dcmix_hr_area, dcmix_hr_volume], [0,1] )
    .combine( Channel.from("dcmix_hr") )
    .combine( mi.map{ tab -> [tab.material, tab.dcmix_hr_area, tab.dcmix_hr_volume] } )
...
process mass_building_total {
    ...
    tuple val(tile), val(state), val(material), 
        file(sdr_lr), file(sdr_mr), file(sdr_hr), 
        file(dcmix_lr), file(dcmix_mr), file(dcmix_hr), 
        file(irh), file(light),
        file(sky), val(pubdir)
